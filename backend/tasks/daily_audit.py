...
from models import Bond, ProductionEntry, BondStatus
...
def run_daily_audit(self, target_date: str = None):
    ...
    results = {"date": str(audit_date), "bonds_processed": 0, "errors": [], "rate_changes": []}
...
def _audit_single_bond(db: Session, bond: Bond, audit_date: date, results: dict):
    ...
    # Step 1: Fetch NASA GHI
    nasa_ghi = asyncio.get_event_loop().run_until_complete(
        nasa_service.get_ghi(float(bond.lat), float(bond.lng), audit_date)
    )
    ...
    # Step 4: Evaluate penalty/recovery
    ...
    # Step 5: Write to blockchain (if rate changed)
    tx_result = None
    if decision.rate_changed:
        ...
        new_status = BondStatus.PENALTY if decision.trigger_type == "PENALTY_TRIGGER" else BondStatus.ACTIVE
    ...
    # Step 6: Send alerts (if rate changed)
    if decision.rate_changed:
