...
def get_alert_digest(
    ...
    """
    Per-bond daily digest: for each bond, returns the last `days` days of
    audit results, blockchain TXes, missing production days, and maturity info.
    Used by the Alert Center tab.
    """
    ...
    bonds = db.query(Bond).filter(Bond.status != BondStatus.MATURED).order_by(Bond.created_at.desc()).all()
    ...
    for bond in bonds:
        ...
        # ── Penalty streak info ───────────────────────────────────────────────
        # Always read streaks from the last COMPLIANT/PENALTY/RECOVERY row —
        # IGNORED rows now store NULL for streak fields (race condition fix).
        latest_real = next(
            (l for l in logs if l.verdict in ("COMPLIANT", "PENALTY", "RECOVERY")),
            None
        )
        penalty_days = int(latest_real.consecutive_penalty or 0) if latest_real else 0
        compliant_days = int(latest_real.consecutive_compliant or 0) if latest_real else 0
